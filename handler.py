"""
Worker GPU para Gaussian Splatting — Pipeline de 8 etapas COMPLETO
Versión: v3 (2026-05-27)

Recibe ZIP de fotos, devuelve .ply + .glb (collision mesh)

Etapas:
  1. Extracción de frames (fotos o video)
  2. Filtro de blur ADAPTATIVO (Laplacian variance + percentiles)
  3. Depth Anything V2 (depth priors, OPCIONAL)
  4. Mask2Former (máscaras vidrios/espejos, OPCIONAL)
  5. COLMAP (poses de cámara, OBLIGATORIO)
  6. gsplat training (Gaussian Splatting, OBLIGATORIO)
  7. Cleanup
  8. splat-transform (collision mesh, OPCIONAL)
"""
...
# Filtro adaptativo (fotos móviles)
...
# ══════════════════════════════════════════════════════════════
# ETAPA 2: FILTRO DE BLUR ADAPTATIVO
# ══════════════════════════════════════════════════════════════

def filter_blur(frames_dir):
    """Filtro adaptativo: nunca elimina TODAS las fotos."""
    ...
    # Calcular varianza Laplaciana de cada foto
    variances = []
    for f in frames:
    ...
    # Nunca eliminar todas
    if len(to_remove) >= len(frames):
        log("WARN: Habríamos eliminado todas las fotos. Conservando todas.", "WARN")
        to_remove = []

    # Conservar al menos MIN_IMGS
    remaining = len(frames) - len(to_remove)
    if remaining < MIN_IMGS:
        log(f"WARN: Solo quedarían {remaining} fotos. Conservando las {MIN_IMGS} mejores.", "WARN")
...
# ══════════════════════════════════════════════════════════════
# ETAPA 3: DEPTH ANYTHING V2 (OPCIONAL)
# ══════════════════════════════════════════════════════════════
...
def gen_masks(frames_dir, masks_dir):
    ...
    try:
        ...
        # ADE20K class indices: mirror=27, windowpane=8, glass=147
        EXCLUDE = {27, 8, 147}
        frames = sorted([f for f in os.listdir(frames_dir) if f.endswith(".jpg")])
        with torch.no_grad():
            for i, f in enumerate(frames):
                ...
                seg = proc.post_process_semantic_segmentation(
                    outputs, target_sizes=[img.size[::-1]])[0].cpu().numpy()
...
def run_colmap(images_dir, output_dir):
    ...
    if not os.path.exists(s0):
        raise RuntimeError("COLMAP no pudo reconstruir la escena. "
                          "Causas posibles: paredes sin textura, motion blur, "
                          "pocas fotos, fotos sin overlap suficiente.")

    # Undistort para gsplat
...
# ══════════════════════════════════════════════════════════════
# ETAPA 6: GSPLAT TRAINING (OBLIGATORIO)
# ══════════════════════════════════════════════════════════════
...
def cleanup_ply(ply_path):
    log("━━━ ETAPA 7: Cleanup ━━━")
    # gsplat ya hace densification/pruning internamente.
    # Modificar el PLY post-training es arriesgado (formato especial).
    log("gsplat ya pruneó outliers durante training. PLY conservado.")
    return ply_path

# ══════════════════════════════════════════════════════════════
# ETAPA 8: COLLISION MESH (OPCIONAL)
# ══════════════════════════════════════════════════════════════
...
def handler(job):
    ...
    Input esperado:
      ...
      Procesamiento:
        {"input": {
          "download_url": "https://...",
          "upload_url_ply": "https://...",      # opcional
          "upload_url_glb": "https://...",      # opcional
    ...
    try:
        ...
        if quality not in ITERS:
            return {"status": "error",
                    "error": f"quality inválido: {list(ITERS.keys())}"}
        iters = ITERS[quality]
        ...
        for d in [raw, frames, colmap, result]:
        ...
        for _ in range(3):
            ...
            if len(items) == 1 and os.path.isdir(os.path.join(raw, items[0])):
                ...
                for f in os.listdir(inner):
                    shutil.move(os.path.join(inner, f), raw)
                os.rmdir(inner)
            else:
                break
        # Subcarpeta "images"
        ...
        if count < MIN_IMGS:
            return {"status": "error", "stage": "extraction",
                    "error": f"Solo {count} frames, mínimo {MIN_IMGS}"}

        # ETAPA 2
        kept = filter_blur(frames)
        if kept < MIN_IMGS:
            return {"status": "error", "stage": "blur",
                    "error": f"Solo {kept} frames nítidos tras filtro, mínimo {MIN_IMGS}"}
        ...
        if upload_ply_url:
            try:
                upload(ply, upload_ply_url)
            except Exception as e:
                log(f"WARN: Upload PLY falló ({e})", "WARN")

        if upload_glb_url and has_glb:
            try:
                upload(glb, upload_glb_url)
            except Exception as e:
        ...
        # Si no hay URL de upload, incluir base64 (solo si pequeño)
        if not upload_ply_url and ply_mb < 30:
            with open(ply, "rb") as f:
                out["ply_base64"] = base64.b64encode(f.read()).decode()
        if not upload_glb_url and has_glb and os.path.getsize(glb) < 5 * 1024 * 1024:
            with open(glb, "rb") as f:
                out["glb_base64"] = base64.b64encode(f.read()).decode()
        ...
        if inp.get("webhook_url"):
            try:
                req.post(inp["webhook_url"], json=err, timeout=10)
            except:
                pass
        return err
        ...
        err = {"status": "error", "stage": "exception", "error": str(e),
               "traceback": tb, "last_cmd": get_cmd(),
               "log": log.tail(), "seconds": elapsed}
        if inp.get("webhook_url"):
            try:
                req.post(inp["webhook_url"], json=err, timeout=10)
            except:
