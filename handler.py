...
def filter_blur(frames_dir):
    ...
    log(f"Varianzas: min={var_min:.1f}, max={var_max:.1f}, media={var_mean:.1f}, mediana={var_median:.1f}")
    ...
    log(f"Sobre umbral absoluto ({BLUR_THRESHOLD_ABSOLUTE}): {above_absolute}/{len(variances)} ({ratio*100:.0f}%)")
    ...
    if ratio >= MIN_VALID_RATIO:
        ...
        log(f"Modo: PERMISIVO (umbral adaptativo = {adaptive_threshold:.1f}, percentil {BLUR_PERCENTILE_FALLBACK})")
    ...
    if remaining_after_remove < MIN_IMGS:
        log(f"WARN: Solo quedarían {remaining_after_remove} fotos. Conservando las {MIN_IMGS} mejores.", "WARN")
...
def gen_masks(frames_dir, masks_dir):
    ...
    try:
        ...
        model = Mask2FormerForUniversalSegmentation.from_pretrained("facebook/mask2former-swin-base-ade-semantic").to(device).eval()
